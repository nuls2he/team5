...
<body>
	...
	<a href="${pageContext.request.contextPath}/adulttoonlist"> 목록</a>
	<a href="${pageContext.request.contextPath}/html/adulttoon/adulttooninsert.jsp">등록</a>
	<table class="table table-hover" >
		...
		<tr>
			...
			<td><a href="${pageContext.request.contextPath}/adulttoondetail?no=${Common.no}"><img src="<c:out value="${pageContext.request.contextPath}/image${Common.imagepath}"/>" width="250" height="200"></a></td>
			<td><c:out value="${Common.title}"/></td>
			<td><c:out value="${Common.content}" /></td>
			<td><c:out value="${Common.regdate}"/>
			<a href="${pageContext.request.contextPath}/adulttooninserttoupdate?no=${Common.no}">수정</a>
			<a href="${pageContext.request.contextPath}/adulttoondelete?no=${Common.no}">삭제</a> </td>
		</tr> 
		</c:forEach>
	</table>	 
			<a href="${pageContext.request.contextPath}/adulttoonlist?rnum=0"><<</a>&nbsp;&nbsp;&nbsp;
			<a href="${pageContext.request.contextPath}/adulttoonlist?rnum=<fmt:formatNumber value="${((rnum/5-(rnum/5%1))-1)*5}" pattern="0"/>"><</a>&nbsp;&nbsp;&nbsp;
			<c:choose>
			<c:when test="${((rnum/5-(rnum/5%1)+1)*5-1)>=(pnum/5)}">
				<c:forEach var="i" begin="${(rnum/5-(rnum/5%1))*5}" end="${pnum/5}">
				<a href="${pageContext.request.contextPath}/adulttoonlist?rnum=${i}">${i+1}</a>&nbsp;&nbsp;&nbsp;
				</c:forEach>
			</c:when>
			<c:otherwise>
			...
			<a href="${pageContext.request.contextPath}/adulttoonlist?rnum=<fmt:formatNumber value="${((rnum/5-(rnum/5%1))+1)*5}" pattern="0"/>">></a>&nbsp;&nbsp;&nbsp;
			<a href="${pageContext.request.contextPath}/adulttoonlist?rnum=<fmt:formatNumber value="${pnum/5}" pattern="0"/>">>></a>&nbsp;&nbsp;&nbsp;
				
		<form name="SearchForm" method="post" action="${pageContext.request.contextPath}/adulttoonlist">
